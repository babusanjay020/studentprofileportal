...
const EXAM_TYPES = ["Mid-Term", "End-Term", "Supplementary", "Internal"];
const EXAM_COLORS = { "Mid-Term": "bg-blue-100 text-blue-800", "End-Term": "bg-purple-100 text-purple-800", "Supplementary": "bg-orange-100 text-orange-800", "Internal": "bg-green-100 text-green-800" };

export default function EditQuestionPapers({ currentUser }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    queryKey: ["papers-all"],
    queryFn: () => db.entities.QuestionPaper.list(),
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => db.entities.QuestionPaper.update(id, {
      ...data,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => db.entities.QuestionPaper.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["papers-all"] });
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 text-slate-400">No question papers found</div>
      ) : (
                ...
                {editingId === record.id ? (
                  <div className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div><Label>Year</Label><Input type="number" value={editData.year} onChange={e => setEditData({ ...editData, year: e.target.value })} /></div>
                      <div>
                        <Label>Exam Type</Label>
                        <Select value={editData.exam_type} onValueChange={v => setEditData({ ...editData, exam_type: v })}>
                          <SelectTrigger><SelectValue placeholder="Type" /></SelectTrigger>
                          <SelectContent>{EXAM_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                        </Select>
                      </div>
                      <div className="md:col-span-3"><Label>File URL</Label><Input value={editData.file_url} onChange={e => setEditData({ ...editData, file_url: e.target.value })} /></div>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-slate-600">{record.subject_name} — Sem {record.semester} — {record.year}</p>
                      {record.department && <p className="text-xs text-indigo-500">{record.department}</p>}
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className={EXAM_COLORS[record.exam_type] || "bg-gray-100"}>{record.exam_type}</Badge>
